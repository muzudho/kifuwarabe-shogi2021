...
func GenControl(pPos *Position, from Square) []Square {
	...
	case PIECE_L1, PIECE_R1, PIECE_PR1, PIECE_R2, PIECE_PR2:
		if Rank(from) > 2 && pPos.IsEmptySq(from-1) { // 1～2段目にある駒でもなく、１つ上が空マスなら
			for to := from - 2; Rank(to) != 0 && pPos.Hetero(from, to); to -= 1 { // 上
				sq_list = append(sq_list, to)
			}
		}
	default:
	...
	case PIECE_R1, PIECE_PR1, PIECE_L2, PIECE_R2, PIECE_PR2:
		if Rank(from) < 8 && pPos.IsEmptySq(from+1) { // 8～9段目にある駒でもなく、１つ下が空マスなら
			for to := from + 2; Rank(to) != 0 && pPos.Hetero(from, to); to += 1 { // 下
				sq_list = append(sq_list, to)
			}
		}
	default:
	...
	case PIECE_R1, PIECE_PR1, PIECE_R2, PIECE_PR2:
		if File(from) < 8 && pPos.IsEmptySq(from+10) { // 8～9筋にある駒でもなく、１つ左が空マスなら
			for to := from + 20; File(to) != 0 && pPos.Hetero(from, to); to += 10 { // 左
				sq_list = append(sq_list, to)
			}
		}
		if File(from) > 2 && pPos.IsEmptySq(from-10) { // 1～2筋にある駒でもなく、１つ右が空マスなら
			for to := from - 20; File(to) != 0 && pPos.Hetero(from, to); to -= 10 { // 右
				sq_list = append(sq_list, to)
			}
		}
	default:
	...
	// 先手桂の動き
	if piece == PIECE_N1 {
		if to := from + 8; File(to) != 0 && to%10 != 0 && to%10 != 9 && pPos.Hetero(from, to) { // 左上桂馬飛び
			sq_list = append(sq_list, to)
		}
		if to := from - 12; File(to) != 0 && to%10 != 0 && to%10 != 9 && pPos.Hetero(from, to) { // 右上桂馬飛び
			sq_list = append(sq_list, to)
		}
	}
	...
	// 後手桂の動き
	if piece == PIECE_N2 {
		if to := from + 12; File(to) != 0 && to%10 != 0 && to%10 != 9 && pPos.Hetero(from, to) { // 左下
			sq_list = append(sq_list, to)
		}
		if to := from - 8; File(to) != 0 && to%10 != 0 && to%10 != 9 && pPos.Hetero(from, to) { // 右下
			sq_list = append(sq_list, to)
		}
	}
	...
	case PIECE_K1, PIECE_R1, PIECE_PR1, PIECE_PB1, PIECE_G1, PIECE_S1, PIECE_L1, PIECE_P1, PIECE_PS1,
		...
		if to := from - 1; Rank(to) != 0 && pPos.Hetero(from, to) { // 上
			sq_list = append(sq_list, to)
		}
	default:
		// Ignored
	...
	case PIECE_K2, PIECE_R2, PIECE_PR2, PIECE_PB2, PIECE_G2, PIECE_S2, PIECE_L2, PIECE_P2, PIECE_PS2,
		...
		if to := from + 1; Rank(to) != 0 && pPos.Hetero(from, to) { // 下
			sq_list = append(sq_list, to)
		}
	default:
		// Ignored
	...
	case PIECE_K1, PIECE_PR1, PIECE_B1, PIECE_PB1, PIECE_G1, PIECE_S1, PIECE_PS1, PIECE_PN1, PIECE_PL1,
		...
		if to := from + 9; File(to) != 0 && Rank(to) != 0 && pPos.Hetero(from, to) { // 左上
			sq_list = append(sq_list, to)
		}
		if to := from - 11; File(to) != 0 && Rank(to) != 0 && pPos.Hetero(from, to) { // 右上
			sq_list = append(sq_list, to)
		}
	default:
		// Ignored
	...
	case PIECE_K2, PIECE_PR2, PIECE_B2, PIECE_PB2, PIECE_G2, PIECE_S2, PIECE_PS2, PIECE_PN2, PIECE_PL2,
		...
		if to := from + 11; File(to) != 0 && Rank(to) != 0 && pPos.Hetero(from, to) { // 左下
			sq_list = append(sq_list, to)
		}
		if to := from - 9; File(to) != 0 && Rank(to) != 0 && pPos.Hetero(from, to) { // 右下
			sq_list = append(sq_list, to)
		}
	default:
		// Ignored
	...
	case PIECE_K1, PIECE_R1, PIECE_PR1, PIECE_PB1, PIECE_G1, PIECE_PS1, PIECE_PN1, PIECE_PL1, PIECE_PP1,
		...
		if to := from + 10; File(to) != 0 && pPos.Hetero(from, to) { // 左
			sq_list = append(sq_list, to)
		}
		if to := from - 10; File(to) != 0 && pPos.Hetero(from, to) { // 右
			sq_list = append(sq_list, to)
		}
	default:
		// Ignored
