...
func GenControl(pPos *Position, boardLayer int, from Square) []Square {
	sq_list := []Square{}
	...
	if from == SQUARE_EMPTY {
		...
		case SQ_P1:
			...
			for rank := Square(start_rank); rank < end_rank; rank += 1 {
				for file := Square(9); file > 0; file-- {
					if !NifuFirst(pPos, boardLayer, file) { // ２歩禁止
		...
		case SQ_P2:
			...
			for rank := Square(start_rank); rank < end_rank; rank += 1 {
				for file := Square(9); file > 0; file-- {
					if !NifuSecond(pPos, boardLayer, file) { // ２歩禁止
						sq_list = append(sq_list, SquareFrom(file, rank))
					}
				}
			}
		...
		case PIECE_B1, PIECE_PB1, PIECE_B2, PIECE_PB2:
			if File(from) < 8 && Rank(from) > 2 && pPos.IsEmptySq(boardLayer, from+9) { // 8～9筋にある駒でもなく、1～2段目でもなく、１つ左上が空マスなら
				for to := from + 18; File(to) != 0 && Rank(to) != 0; to += 9 { // ２つ左上から
					sq_list = append(sq_list, to)
					if !pPos.IsEmptySq(boardLayer, to) {
						break
					}
				}
			}
			if File(from) > 2 && Rank(from) > 2 && pPos.IsEmptySq(boardLayer, from-11) { // 1～2筋にある駒でもなく、1～2段目でもなく、１つ右上が空マスなら
			...
			if File(from) < 8 && Rank(from) < 8 && pPos.IsEmptySq(boardLayer, from+11) { // 8～9筋にある駒でもなく、8～9段目でもなく、１つ左下が空マスなら
			...
			if File(from) > 2 && Rank(from) < 8 && pPos.IsEmptySq(boardLayer, from-9) { // 1～2筋にある駒でもなく、8～9段目でもなく、１つ右下が空マスなら
		...
		case PIECE_L1, PIECE_R1, PIECE_PR1, PIECE_R2, PIECE_PR2:
			if Rank(from) > 2 && pPos.IsEmptySq(boardLayer, from-1) { // 1～2段目にある駒でもなく、１つ上が空マスなら
		...
		case PIECE_R1, PIECE_PR1, PIECE_L2, PIECE_R2, PIECE_PR2:
			if Rank(from) < 8 && pPos.IsEmptySq(boardLayer, from+1) { // 8～9段目にある駒でもなく、１つ下が空マスなら
		...
		case PIECE_R1, PIECE_PR1, PIECE_R2, PIECE_PR2:
			if File(from) < 8 && pPos.IsEmptySq(boardLayer, from+10) { // 8～9筋にある駒でもなく、１つ左が空マスなら
			...
			if File(from) > 2 && pPos.IsEmptySq(boardLayer, from-10) { // 1～2筋にある駒でもなく、１つ右が空マスなら
...
func NifuFirst(pPos *Position, boardLayer int, file Square) bool {
	for rank := Square(2); rank < 10; rank += 1 {
		if pPos.Board[boardLayer][SquareFrom(file, rank)] == PIECE_P1 {
			return true
		}
	}
...
func NifuSecond(pPos *Position, boardLayer int, file Square) bool {
	for rank := Square(1); rank < 9; rank += 1 {
		if pPos.Board[boardLayer][SquareFrom(file, rank)] == PIECE_P2 {
			return true
		}
	}
...
func GenMoveList(pPos *Position, boardLayer int) []Move {
	...
	if pPos.ControlBoards[opponent-1][CONTROL_LAYER_SUM][friendKingSq] > 0 {
		...
		for rank := 1; rank < 10; rank += 1 {
			for file := 1; file < 10; file += 1 {
				...
				if pPos.Homo(boardLayer, from, friendKingSq) { // 自玉と同じプレイヤーの駒を動かします
					control_list := GenControl(pPos, boardLayer, from)
					...
					if pieceType == PIECE_TYPE_K {
						...
						for _, to := range control_list {
							...
							if pPos.Hetero(boardLayer, from, to) { // 自駒の上には移動できません
		...
		for hand_index := hand_start; hand_index < hand_end; hand_index += 1 {
			if pPos.Hands[boardLayer][hand_index] > 0 {
				hand_sq := Square(hand_index) + SQ_HAND_START
				control_list := GenControl(pPos, boardLayer, hand_sq)

				for _, to := range control_list {
					if pPos.IsEmptySq(boardLayer, to) { // 駒の上には打てません
			...
			for file := 1; file < 10; file += 1 {
				...
				if pPos.Homo(boardLayer, from, friendKingSq) { // 自玉と同じプレイヤーの駒を動かします
					...
					if pieceType == PIECE_TYPE_K {
						// 玉は自殺手を省きます
						for _, to := range control_list {
							if pPos.Hetero(boardLayer, from, to) && pPos.ControlBoards[opponent-1][CONTROL_LAYER_SUM][to] == 0 { // 自駒の上、敵の利きには移動できません
